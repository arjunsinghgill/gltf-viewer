...
const HTTPS_binary_files = {
  sample : fs.readFileSync( path.resolve( __dirname, './sample.glb' ) )
};

const HTTPS_ROUTE_home_files = {
  html : fs.readFileSync( path.resolve(__dirname,'./src.gltf_viewer.html') ),
  js   : fs.readFileSync( path.resolve(__dirname,'./src.gltf_viewer.js') ),
  css  : fs.readFileSync( path.resolve(__dirname,'./src.gltf_viewer.css') ),
};
...
const HTTPS_ROUTES = {
  ...
  '/binary/sample.glb': async function( IN_req, IN_res, IN_url_parsed) {
    IN_res.setHeader('Content-Type','model/gltf-binary');
    IN_res.writeHead(200);
    IN_res.end( HTTPS_binary_files.sample );
  }
};
...
const HTTPS_TLS_SERVER = https.createServer( HTTPS_TLS_CERTS, async (req,res) => {
  ...
  try {
    // parse the url
    let TMP_url_parsed = new URL( req.url, "https://localhost:8082/" );
    // store the pathname
...
console.log('HTTPS server running at port : 8082');
